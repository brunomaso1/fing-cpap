...
from .data.loader import create_segmentation_dataloaders, get_voc_class_names
...
def main():
    ...
    # Use VOC Segmentation dataset instead
    train_loader, val_loader, test_loader, num_classes = create_segmentation_dataloaders(
        config=config,
        root='./data/VOC'
    )
    
    class_names = get_voc_class_names()
    config.N_CLASSES = num_classes
    
    print(f"\n✓ Data loading complete!")
    # print(f"  Classes: {class_names}")
    # print(f"  Total samples: {len(images)}")
    print(f"  Classes: {len(class_names)} segmentation classes")
